...
```{python phenotype visualiseren , echo=FALSE, message=FALSE, warning=FALSE, fig.cap= phenotype in gene expression UMAP}
...
De genen App.As2, Mbp.As3 en Emc10 worden hier weergeven omdat deze biologisch relevant zijn. Dit zijn namelijk genen die betrokken zijn bij eiwit vouwing en myeline afbraak en dus een belangrijk onderdeel van het centrale zenuwstelsel. Het zien of er verschil is in deze genen bij gezond weefsel en "Alzheimer" weefsel kan biologisch zeer intressant zijn. 
...
In de UMAPs wordt weergeven wat de verdeling van PSI waarden is van de 3 genen. 
...
Hierin wordt de PSI per cluster weergeven. 
```{python violin plots genen , echo=FALSE, message=FALSE, waring=FALSE, fig.cap= violin plot PSI}
...
```
Deze UMAP laat de verdeling van cellen zien. 
...
```{python variance ratio, echo=FALSE, message=FALSE, warning=FALSE, fig.cap= variance ratio}
...
```{python UMAP PSI combine , echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, fig.cap= Visualisatie PSI value }
