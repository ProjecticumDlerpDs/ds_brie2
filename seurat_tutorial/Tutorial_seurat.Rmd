...
```{r data set in feature scatter , echo=FALSE, fig.align='center', fig.height=5, fig.width=15, fig.cap= correlatie dataset }
...
print(combined)
```
In deze afbeelding zien we op de Y-as het percentage MT en nFeature tegen nCount op de X-as. 
...
Op de Y-as wordt de variatie (verschil in expressie) weergeven, en op de X-as de gemiddelde expressie. 
```{r visualisatie variabele genen, echo=FALSE, fig.align='center', fig.height=8, fig.width=10, message=FALSE, warning=FALSE, fig.cap= Variable genen}
...
Deze PCA analyse kan op verschillende manieren worden gevisualiseerd. Hieronder zullen een aantal voorbeelden langskomen. 
Als eerste een VizDim waarin de genen die verantwoordelijk zijn voor een PC worden weergeven. 
```{r VizDIM plot PCA, echo=FALSE, fig.cap= VizDim plot PCA analyse}
VizDimLoadings(pbmc, dims = 1:2, reduction = "pca")
```
Op de Y-as worden de namen van de genen weergeven en op de X-as wordt loading waarden weergeven. Dit betekend hoeveel het gen bijdraagd aan een PC. Zo kan je bijvoorbeeld kijken of PC's genen bevatten die biologisch relevant zijn. 

Dan volgt de Dimplot. Hierin wordt de data structuur weergeven. 
Op de Y-as staat tweede hoofdimentie en op de X-as de eerste hoofdementie. In deze weergaven is iedere stip 1 cel. Hieruit kan gezien worden of de PC goed van elkaar gescheiden zijn of juist overlappen. 
```{r Dimplot PCA analyse, echo=FALSE, fig.cap= Dimplot PCA analyse}
DimPlot(pbmc, reduction = "pca") + NoLegend()
```

Als laatste wordt de Dim Heatmap weergeven. Deze visualisatie geeft een overzicht van welke genen belangrijk zijn voor welk PC en hoe deze tot expressie komen in alle genen. 
De rijen zijn de genen met hoge loading (bijdragen PC) en de kolommen zijn de cellen. De kleur gele kleur laat de expressie zien. 
```{r Heatmap PCA analyse, echo=FALSE, fig.height=15, fig.width=20, fig.cap= Heatmap PCA analyse}
DimHeatmap(pbmc, dims = 1:15, cells = 500, balanced = TRUE)
```
Deze Heatmap kan laten zien welke genen invloed hebben op de PC's, kijken of er bepaalde clusters of celtype een verschillend patroon hebben of welke PC je wilt gebruiken voor verdere analyse.  
...
```{r Biomarkers vinden , echo=FALSE, message=FALSE}
...
```{r Heatmap Markers, echo=FALSE, fig.align='center', fig.height=15, fig.width=20, message=FALSE, warning=FALSE, fig.cap=Heatmap top 10 markers }
pbmc.markers %>%
    group_by(cluster) %>%
    dplyr::filter(avg_log2FC > 1) %>%
    slice_head(n = 10) %>%
...
```{r celtype koppelen , include=FALSE}
...
```{r UMAP met celtype, echo=FALSE, message=FALSE, warning=FALSE, fig.cap= UMAP met celtype }
library(ggplot2)
